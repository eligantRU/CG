...
{                                    
	...
	//color += 0 * texture2D(normalmap, fragTextureUV.st);
	vec4 diffuseIntensity = color * light0.diffuse;
	vec4 specularIntensity = factors.specular * light0.specular; 

    vec4 DiffuseColor = color; // TODO
    vec4 AmbientColor = color; // TODO
    ...
    vec4 LightColor = vec4(1, 1, 1, 1); // TODO
    ...
    gl_FragColor = color * vec4(FinalColor, 1.0) + 0.5 * diffuseIntensity + 0.2 * specularIntensity; //color * vec4(FinalColor, DiffuseColor.a);
}

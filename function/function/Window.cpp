...
{
	for (float x = windowSize.x / 2; x <= windowSize.x; x += windowSize.x * 2 * SCALE.x )
	{
	...
	for (float y = windowSize.y / 2; y <= windowSize.y; y += windowSize.y * SCALE.y)
	{
...
CWindow::CWindow()
{
    SetBackgroundColor({1.f, 1.0f, 1.0f, 1.f});
}
